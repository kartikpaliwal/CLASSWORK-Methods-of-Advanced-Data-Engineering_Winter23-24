import os
import pandas as pd
from data_engineering_pipeline import download_dataset, extract_zip, process_csv_files

def test_download_dataset(dataset_dir, competition_name):
    download_dataset(competition_name, dataset_dir)
    zip_file_exists = os.path.exists(os.path.join(dataset_dir, competition_name + '.zip'))
...
def test_integrity(csv_file_path):
    ...
    assert not df['timestamp'].isnull().any(), "Data Integrity Failed: Null values found in 'timestamp' column"
...
def run_tests():
    ...
    test_extract_zip(os.path.join(dataset_dir, competition_name + '.zip'), dataset_dir)
